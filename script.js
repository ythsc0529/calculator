document.addEventListener('DOMContentLoaded', () => {
    ...
    let purificationGame = {
        sequence: [],
        currentIndex: 0,
        clicksNeeded: 15,
        currentClicks: 0,
        timePerTile: 3000, // 每張牌3秒
        timerId: null,
        isActive: false
    };
    ...
    const purificationTilesContainer = document.getElementById('purification-tiles');
    const purificationProgressContainer = document.getElementById('purification-progress-container');
    const purificationProgressBar = document.getElementById('purification-progress-bar');
    ...
    function setupFortune() {
        handleDailyFortune();
        startRitualBtn.addEventListener('click', startPurificationGame);
        checkBlessing();
    }
    ...
    function startPurificationGame() {
        if (purificationGame.isActive) return;
        purificationGame.isActive = true;

        startRitualBtn.style.display = 'none';
        summonResultEl.innerHTML = '<strong style="color:blue;">儀式開始！準備淨化！</strong>';
        
        let deck = [...ALL_TILES];
        purificationGame.sequence = [];
        for (let i = 0; i < 4; i++) { // 這次玩4張牌
            purificationGame.sequence.push(deck.splice(Math.floor(Math.random() * deck.length), 1)[0]);
        }
        
        purificationTilesContainer.innerHTML = '';
        purificationGame.sequence.forEach(tileName => {
            const tileEl = createTileImage(tileName);
            purificationTilesContainer.appendChild(tileEl);
        });

        purificationGame.currentIndex = 0;
        setTimeout(() => {
            startNextTilePurification();
        }, 1000);
    }
    function startNextTilePurification() {
        if (purificationGame.currentIndex >= purificationGame.sequence.length) {
            handleGameSuccess();
            return;
        }
        purificationProgressContainer.classList.add('visible');
        purificationGame.currentClicks = 0;
        updateProgressBar();
        const currentTileEl = purificationTilesContainer.children[purificationGame.currentIndex];
        currentTileEl.classList.add('purifying');
        
        const clickHandler = () => {
            purificationGame.currentClicks++;
            updateProgressBar();
            if (purificationGame.currentClicks >= purificationGame.clicksNeeded) {
                clearTimeout(purificationGame.timerId);
                currentTileEl.removeEventListener('click', clickHandler);
                currentTileEl.classList.remove('purifying');
                currentTileEl.classList.add('purified');
                purificationGame.currentIndex++;
                startNextTilePurification();
            }
        };
        currentTileEl.addEventListener('click', clickHandler);

        purificationGame.timerId = setTimeout(() => {
            currentTileEl.removeEventListener('click', clickHandler);
            handleGameFailure();
        }, purificationGame.timePerTile);
    }
    function updateProgressBar() {
        const percentage = (purificationGame.currentClicks / purificationGame.clicksNeeded) * 100;
        purificationProgressBar.style.width = `${percentage}%`;
    }
    function handleGameSuccess() {
        purificationGame.isActive = false;
        purificationProgressContainer.classList.remove('visible');
        summonResultEl.innerHTML = '<strong style="color:var(--gold-color);">淨化成功！財神賜予您12小時的福運！</strong>';
    ...
    function handleGameFailure() {
        ...
        purificationProgressContainer.classList.remove('visible');
        purificationTilesContainer.innerHTML = '';
        summonResultEl.innerHTML = '<strong style="color:red;">淨化失敗！</strong>能量消散了，請重新開始儀式。';
        startRitualBtn.style.display = 'block';
    }
