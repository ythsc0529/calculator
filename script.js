document.addEventListener('DOMContentLoaded', () => {
    ...
    function startGame() {
        ...
        const p4Name = document.getElementById('player4-name').value || '北方玩家';
    ...
    function handleSettle() {
        ...
        if (transactions.length === 0) {
            ...
            transactions.forEach(t => {
                content += `<li><span class="player-name">${t.from}</span><span class="transaction-arrow"> → </span><span class="player-name">${t.to}</span><span class="transaction-amount">${t.amount} 點</span></li>`;
            });
            content += '</ul>';
        }
        content += '</div>';
    ...
    function setupFortune() {
        handleDailyFortune();
        gameChoiceBtns.forEach(btn => btn.addEventListener('click', () => showGame(btn.dataset.game)));
        backToSelectionBtns.forEach(btn => btn.addEventListener('click', () => showGame('selection')));
        ...
        setupDivinationGame();
        setupSequenceGame();
        setupPurificationGame();
        setupDragonPearlGame();
        setupMemoryMatchGame();
        setupChosenOneGame();
        setupWindRisesGame();
        setupScoopTheMoonGame();

        checkBlessing();
    }
    function showGame(gameName) {
        gameSelectionMenu.style.display = 'none';
        ritualGameContainers.forEach(c => c.style.display = 'none');

        if (gameName === 'selection') {
    ...
    function checkBlessing() {
        ...
        if (!blessingEnd || Date.now() > blessingEnd) {
            gameHallContainer.style.display = 'block';
            blessingCountdownArea.style.display = 'none';
            if (blessingTimerId) clearInterval(blessingTimerId);
            return false;
        }
        
        gameHallContainer.style.display = 'none';
        blessingCountdownArea.style.display = 'block';
        
        const updateTimer = () => {
            const remaining = blessingEnd - Date.now();
            if (remaining <= 0) {
                clearInterval(blessingTimerId);
                localStorage.removeItem('blessingExpiry');
                checkBlessing(); // 重新檢查以顯示遊戲廳
                location.reload(); // 簡單粗暴的方式刷新狀態
                return;
            }
            const hours = Math.floor(remaining / 3600000);
            const minutes = Math.floor((remaining % 3600000) / 60000);
            const seconds = Math.floor((remaining % 60000) / 1000);
            blessingCountdownArea.innerHTML = `財神庇佑中！福運剩餘時間：<br><strong style="color:var(--gold-color);">${hours}時 ${minutes}分 ${seconds}秒</strong>`;
        };
        updateTimer();
    ...
    function handleGameWin(message) {
        ...
        const godIcon = document.querySelector('.god-of-wealth');
        if (godIcon) {
            godIcon.classList.add('success-glow');
            setTimeout(() => godIcon.classList.remove('success-glow'), 1500);
        }
        alert(message);
        checkBlessing();
    }
    ...
    function spinReels() { if (slotGame.isSpinning) return; slotGame.spinsLeft--; spinsLeftEl.textContent = slotGame.spinsLeft; slotGame.isSpinning = true; spinBtn.disabled = true; slotResultEl.innerHTML = ''; const reels = [reel1, reel2, reel3]; const results = reels.map(() => ALL_TILES[Math.floor(Math.random() * ALL_TILES.length)]); reels.forEach((reel, index) => { populateReel(reel, results[index]); reel.style.transition = 'none'; reel.style.transform = 'translateY(0)'; reel.getBoundingClientRect(); const spinDuration = 2 + index * 0.5; reel.style.transition = `transform ${spinDuration}s cubic-bezier(0.25, 1, 0.5, 1)`; const finalPosition = 25 * slotGame.tileHeight; reel.style.transform = `translateY(-${finalPosition}px)`; }); setTimeout(() => { checkSlotWin(results); }, 3500); }
    function checkSlotWin(results) { slotGame.isSpinning = false; let isWin = false; let winMessage = ""; const [t1, t2, t3] = results; if (t1 === t2 && t1 === t3) { isWin = true; winMessage = `大獎！三條 ${t1}！`; } else if (['中', '發', '白'].sort().join(',') === [...results].sort().join(',')) { isWin = true; winMessage = "中獎！三元會首！"; } else { const parsed = results.map(parseTileForSlot).sort((a, b) => a.num - b.num); if (parsed[0].suit && parsed[0].suit === parsed[1].suit && parsed[0].suit === parsed[2].suit && parsed[1].num === parsed[0].num + 1 && parsed[2].num === parsed[1].num + 2) { isWin = true; winMessage = `中獎！順子 ${results.sort().join(' ')}！`; } } if (isWin) { handleGameWin(`${winMessage} 招財成功！`); } else if (slotGame.spinsLeft > 0) { slotResultEl.innerHTML = '可惜，差一點！'; spinBtn.disabled = false; } else { slotResultEl.innerHTML = '<strong style="color:red;">機會用完了！</strong><br><button id="reset-slot-btn">再試一次</button>'; document.getElementById('reset-slot-btn').onclick = ()=>{ slotGame.spinsLeft = 5; spinsLeftEl.textContent = 5; spinBtn.disabled = false; slotResultEl.innerHTML = ''; }; } }
    ...
    // --- 遊戲7：對對碰 ---
    function setupMemoryMatchGame(){let startMemoryMatchBtn=document.createElement("button");startMemoryMatchBtn.textContent="開始記憶",memoryMatchResultEl.appendChild(startMemoryMatchBtn),startMemoryMatchBtn.addEventListener("click",startMemoryMatch)}
