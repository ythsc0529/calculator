document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkBlessing() {
        ...
        const updateTimer = () => {
            ...
            if (remaining <= 0) { clearInterval(blessingTimerId); location.reload(); return; }
    ...
    function spinReels() { if (slotGame.isSpinning) return; slotGame.isSpinning = true; spinBtn.disabled = true; slotResultEl.innerHTML = ''; const reels = [reel1, reel2, reel3]; const results = reels.map(() => ALL_TILES[Math.floor(Math.random() * ALL_TILES.length)]); reels.forEach((reel, index) => { populateReel(reel, results[index]); reel.style.transition = 'none'; reel.style.transform = 'translateY(0)'; reel.getBoundingClientRect(); const spinDuration = 2 + index * 0.5; reel.style.transition = `transform ${spinDuration}s cubic-bezier(0.25, 1, 0.5, 1)`; const finalPosition = 25 * slotGame.tileHeight; reel.style.transform = `translateY(-${finalPosition}px)`; }); setTimeout(() => { checkSlotWin(results); }, 3500); }
    function checkSlotWin(results) { slotGame.isSpinning = false; spinBtn.disabled = false; let isWin = false; let winMessage = ""; const [t1, t2, t3] = results; if (t1 === t2 && t1 === t3) { isWin = true; winMessage = `大獎！三條 ${t1}！`; } else if (['中', '發', '白'].sort().join(',') === [...results].sort().join(',')) { isWin = true; winMessage = "中獎！三元會首！"; } else { const parsed = results.map(parseTileForSlot).sort((a, b) => a.num - b.num); if (parsed[0].suit && parsed[0].suit === parsed[1].suit && parsed[0].suit === parsed[2].suit && parsed[1].num === parsed[0].num + 1 && parsed[2].num === parsed[1].num + 2) { isWin = true; winMessage = `中獎！順子 ${results.sort().join(' ')}！`; } } if (isWin) { handleGameWin(`${winMessage} 招財成功！`); } else { slotResultEl.innerHTML = '可惜，差一點！再來一次！'; } }
    ...
    // --- 遊戲7：對對碰 ---
    function setupMemoryMatchGame(){startMemoryMatchBtn=document.createElement("button"),startMemoryMatchBtn.textContent="開始記憶",memoryMatchResultEl.appendChild(startMemoryMatchBtn),startMemoryMatchBtn.addEventListener("click",startMemoryMatch)}
